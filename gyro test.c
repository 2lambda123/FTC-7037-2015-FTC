...
//				while(ProcessValue<Target)//hold execution flow
//until we get to target
//				{
//					writeDebugStreamLine("checking Gyro:%d",
//ProcessValue); 					wait1Msec(10);// 0.01 seconds 					int
//CurrentReading=(SensorValue[gyro]*10)-InitVal; 					ProcessValue
//=(-1*(ProcessValue+CurrentReading)* 0.01);//distance = rate*time
//				}
...
void Drive(float outputMagnitude, float curve) //(-1,+1) all
{
  ...
  writeDebugStreamLine(
      ...
      leftOutput * 100, rightOutput * 100, value, ratio);
}
void Drive(int Distance) {
  ...
  if (Distance > 0) // forward
  {
    ...
    while (Target > nMotorEncoder[leftMotor]) {
      ...
      int Y = enc_in *
              (MAX(Lenc, Renc) -
               MIN(Lenc, Renc));   // the length of difference betwwen encoders
      int H = WheelbaseRadius * 2; // longest leg of the triangle
      int X =
          sqrt(Y ^ 2 + H ^ 2) * direction; // distance from horizontal square.
