...
int GoalPosition = 0; // must be initialized as zero so it can overide when/if
                      // LocateInfaRedBeacon() produces no change
...
void RunLift(float nStage) {
  float Range = 22.5; // mechanical rotations of the motor shaft to extend full
                      // length of lift
  ...
  writeDebugStreamLine("Target: %f, nStage: %i", nTarget,
                       nStage); // nStage is reading but nTarget is still 0???
  while (true) {
    ...
    // are we there yet? if(Target>position) move towards target else{stop and
    // break;}
  }
  ...
  if (nTarget > 1) // run up
  {
    while (nMotorEncoder[Lift] <= nTarget) {
      ...
      writeDebugStreamLine("Value :%f Target: %f, Location: %f",
                           nMotorEncoder[Lift], nTarget, nFeedBack);
      motor[Lift] = 75;
    }
    float nFeedBack = (nMotorEncoder[Lift] / 1120) / 22.5;
    writeDebugStreamLine("Value :%f Target: %f, Location: %f",
  ...
  if (nTarget < 1) // run down
  {
    ...
    {
      writeDebugStreamLine("Value :%i, Target: %i", nMotorEncoder[Lift],
                           nTarget);
      motor[Lift] = -75;
    }
    motor[Lift] = 0;
...
void Drive(int distance) {
  ...
  if (distance > 1) {
    while (nMotorEncoder[motorE] <= Target) {
      writeDebugStreamLine("Value :%i, Target: %i", nMotorEncoder[motorE],
                           Target);
    }
    motor[motorD] = 0;
    motor[motorE] = 0;
