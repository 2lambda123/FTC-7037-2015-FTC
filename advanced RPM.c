...
void Drive(int TargetRPM) {
  ...
  if (TargetRPM <
      MotorSpecRPM *
          0.75) // make sure we are within the limitations of the motor
  {
    while (true) {
      ...
      int PulseFreqeuncy =
          nMotorEncoder[Right] / TimePassed * 100; /*pulses/seconds in HZ*/
      // get wheel rpm
      int RPM = (PulseFreqeuncy * 60) /
                MotorSpecPPR; //(pulse/sec*60sec/min)/PPR = R/M
      writeDebugStream("\n RPM:%i, TIME:%i, PulseFreqeuncy:%i", RPM,
                       TimePassed / 100, PulseFreqeuncy);
      wait10Msec(50);
      clearDebugStream();
      ////set motor speeds
