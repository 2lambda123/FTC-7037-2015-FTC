...
void initializeRobot() { servo[Gripper] = 80; }
// void getUserInput()
//{
...
void RunLift(short nStage) {
  ...
  if (hasbeeninit == 0) // initialize the lift
  {
    ...
    while (true) {
      ...
      if (inputdata ==
          -1) // this is the fault state for the HTSPB reading analog.
      {
        // report error
  ...
  while (abs(nTarget - nMotorEncoder[Lift]) >
         Tolerance + 5) //--TEST added 5 becuase Tolerance to low?
  {
...
void Drive(int distance, int power) {
  ...
  while (nMotorEncoder[Right] != Target || nMotorEncoder[Left] != Target) {
    ...
    {
      if (((Lspeed + Rspeed) / 2) >
          (-1 * power)) // slowdown/ reverse until full speed backwards
      {
        Lspeed - 1;
        Rspeed - 1;
      }
