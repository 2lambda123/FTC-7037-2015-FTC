...
void Drive(int distance) {
  ...
  while (true) {
    ...
    writeDebugStreamLine("PID:%i, Target:%i,	Left:%i,	Right:%i",
                         PID(Target, nMotorEncoder[Left]), Target,
                         -1 * nMotorEncoder[Left], nMotorEncoder[Right]);
  }
  // Motor[Left]= PID( nMotorEncoder[Right], nMotorEncoder[Left]); //set the
  // other as slave, one encoder will be backward, FIX ME
