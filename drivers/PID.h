...
Sue Shields-- helping me find the "dependant" form of I, this greatly simplified
the process. David Shiels-- demonstrating how to tune The PID controller.
*/
...
float PID(float setpoint, float ProcessValue) {
  ...
  // Update error, this program has to be run in a loop so that error can be
  // corected over time
  init_error = error;
  return PIDoutput;
}
